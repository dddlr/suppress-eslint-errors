...
Pass a custom `.eslintrc.js` configuration file:

```bash
npx suppress-eslint-errors ./src --extensions=ts,tsx --parser=tsx --rules=eqeqeq,@typescript-eslint/no-explicit-any --configPath ~/my-repo/.eslintrc.js
```
